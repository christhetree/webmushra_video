...
function MushraPage(_pageManager, _pageTemplateRenderer, _audioContext, _bufferSize, _audioFileLoader, _session, _pageConfig, _mushraValidator, _errorHandler, _language) {
	this.isMushra = true; 
  ...
  this.hasPlayedAllConditions = false;
  this.hasMovedAllSliders = false;
  this.movedSliders = new Set();
  this.repeatingInterval = null;
  this.referenceRatingCloseTo50 = false;
  this.ratingRangeValid = false;
  this.playedConditions = new Set();

  this.videoPlayer = null;
  this.videoUrl = this.pageConfig.video;
  this.videoWidth = 1280;
  if ("videoWidth" in this.pageConfig) {
    this.videoWidth = this.pageConfig.videoWidth;
  }
  this.videoHeight = 720;
  if ("videoHeight" in this.pageConfig) {
    this.videoHeight = this.pageConfig.videoHeight;
  }
}
...
MushraPage.prototype.init = function () {
  ...
  if (_event.name == 'stopTriggered') {
    ...
    if (this.videoPlayer && this.videoPlayer.canPlay()) {
      this.videoPlayer.stop();
    } else if (this.videoPlayer) {
      alert("Video is loading, please be patient.");
    }
  } else if (_event.name == 'playReferenceTriggered') {
    this.playedConditions.add(-1);  // Use -1 as the index for the reference recording
    console.log("hasPlayedAllConditions: " + this.hasPlayedAllConditions);
    console.log("this.playedConditions: " + Array.from(this.playedConditions.values()));
    let nConditions = this.mushraAudioControl.getConditions().length + 1;  // Add one for reference
    if (this.playedConditions.size >= nConditions) {
      this.hasPlayedAllConditions = true;
    }
    this.lockOrUnlockNextButton();
    ...
    this.playedConditions.add(index);
    console.log("hasPlayedAllConditions: " + this.hasPlayedAllConditions);
    console.log("this.playedConditions: " + Array.from(this.playedConditions.values()));
    let nConditions = this.mushraAudioControl.getConditions().length + 1;  // Add one for reference
    if (this.playedConditions.size >= nConditions) {
...
MushraPage.prototype.render = function (_parent) {
  var div = $("<div></div>");
  _parent.append(div);
  _parent.append("<table align='center'><tr><td><strong>Comments (optional):</strong></td><td><input id='trialComments'/></td></tr></table>");

  if (this.videoUrl) {
    var videoDiv = $('<video id="video" muted width="' + this.videoWidth + '" height="' + this.videoHeight + '"></video>');
    _parent.append(videoDiv);
  }
  if (this.pageConfig.mustRate) {
    // _parent.append("<p>The reference and all conditions must be played at least once each before being able to continue. You do not have to listen to each recording in its entirety.</p>");
    _parent.append("<p>The reference and all conditions must be played at least once each before being able to continue.</p>");
  }
  if (this.pageConfig.mustMoveSliders) {
    _parent.append("<p>You must click each rating slider at least once each before being able to continue.</p>");
  }
  if (this.pageConfig.checkReference) {
    _parent.append("<p>During training, one of the conditions is the same as the reference. Since you are comparing the conditions to the reference, this hidden reference condition must be rated between 0 and 25 to proceed (0 = no audible difference from the reference).</p>");
  }
  if (this.pageConfig.checkRange) {
    _parent.append("<p>During training, you must use the full range of the rating scale to proceed: your lowest rating must be between 0 and 25, and your highest rating must be between 75 and 100.</p>");
  }
  var content;
  ...
  for (i = 0; i < conditions.length; ++i) {
    td = $("<td class='spaceForSlider'> \
      <span><input type='range' name='"+conditions[i].getId()+"' class='scales' value='0' min='0' max='100' data-vertical='true' data-highlight='true' style='display : inline-block; float : none;'/></span> \
    </td>");
...
MushraPage.prototype.renderCanvas = function(_parentId) {
  ...
  canvasContext.fillText("50", xTextScoreRanges, prevScalesHeight * 0.50 + YfirstLine);
...
MushraPage.prototype.lockOrUnlockNextButton = function () {
  var canProceed = true;

  if (this.pageConfig.mustRate && !this.hasPlayedAllConditions) {
    canProceed = false;
  }
  if (this.pageConfig.mustMoveSliders && !this.hasMovedAllSliders) {
    canProceed = false;
  }
  if (this.pageConfig.checkReference && !this.referenceRatingCloseTo50) {
    canProceed = false;
  }
  if (this.pageConfig.checkRange && !this.ratingRangeValid) {
    canProceed = false;
  }
...
MushraPage.prototype.checkReferenceRating = function (parent) {
  var scales = $(".scales");

  for (var idx = 0; idx  < scales.length; ++idx) {
    let scale = scales[idx]
    if (scale.name === "reference") {
      let value = scale.value
      // console.log("repeater value = " + value);
      if (0 <= value && value <= 25) {
        parent.referenceRatingCloseTo50 = true;
      } else {
        parent.referenceRatingCloseTo50 = false;
      }
    }
  }

  // console.log("repeater referenceRatingCloseTo50 = " + parent.referenceRatingCloseTo50);
  parent.lockOrUnlockNextButton();
}

MushraPage.prototype.checkRangeRating = function (parent) {
  var scales = $(".scales");
  var minValue = 100;
  var maxValue = 0;

  for (var idx = 0; idx < scales.length; ++idx) {
    var value = parseInt(scales[idx].value);
    if (value < minValue) minValue = value;
    if (value > maxValue) maxValue = value;
  }

  parent.ratingRangeValid = (minValue <= 25 && maxValue >= 75);
  parent.lockOrUnlockNextButton();
}
...
MushraPage.prototype.load = function () {
  ...
  this.lockOrUnlockNextButton();
  if (this.pageConfig.checkReference) {
    this.repeatingInterval = setInterval(this.checkReferenceRating, 500, this);
  }
  if (this.pageConfig.checkRange) {
    this.rangeInterval = setInterval(this.checkRangeRating, 500, this);
  }
  if (this.pageConfig.mustMoveSliders) {
    var self = this;
    var nConditions = this.conditions.length;
    var markSliderMoved = function (index) {
      console.log("Slider moved: index=" + index + " name=" + $(".scales").get(index).name + " value=" + $(".scales").get(index).value);
      self.movedSliders.add(index);
      console.log("movedSliders: " + Array.from(self.movedSliders.values()) + " (" + self.movedSliders.size + "/" + nConditions + ")");
      if (self.movedSliders.size >= nConditions) {
        self.hasMovedAllSliders = true;
      }
      self.lockOrUnlockNextButton();
    };
    $(".scales").on('change', function () {
      markSliderMoved($(".scales").index(this));
    });
    $(".spaceForSlider").on('mousedown touchstart', function () {
      markSliderMoved($(".spaceForSlider").index(this));
    });
  }
  ...
  if (this.ratings.length !== 0) {
    ...
    // If navigating back to a previously rated page, mark all sliders as moved
    if (this.pageConfig.mustMoveSliders) {
      for (i = 0; i < scales.length; ++i) {
        this.movedSliders.add(i);
      }
      this.hasMovedAllSliders = true;
    }
  }
  if (this.trialComments) {
    $("#trialComments").val(this.trialComments);
  }
  ...
  if (this.videoUrl) {
    this.videoPlayer = new VideoPlayer("video", this.videoUrl, "video/mp4", this.mushraAudioControl);
    this.videoPlayer.load();
  } else {
    console.log("no video URL provided: " + this.videoUrl);
  }
};
...
MushraPage.prototype.save = function () {
  ...
  console.log("prev total time on page = " + this.time);
  var timeOnPage = (saveTime - this.startTimeOnPage);
  console.log("time on page = " + timeOnPage);
  if (this.videoPlayer && this.videoPlayer.timeFinishedLoading) {
    timeOnPage = (saveTime - this.videoPlayer.timeFinishedLoading);
    console.log("time on page (after loading video) = " + timeOnPage);
  }
  this.time += timeOnPage;
  console.log("total time on page = " + this.time);
  ...
  let trialComments = $("#trialComments").val();
  if (trialComments) {
...
MushraPage.prototype.store = function () {
  ...
  trial.comments = this.trialComments;
  var i;
