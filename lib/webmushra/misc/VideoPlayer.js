...
VideoPlayer.prototype.load = function () {
    ...
    $.mobile.loading("show", {
        text : "Loading video please be patient...",
        textVisible : true,
        theme : "a",
        html : ""
    });
    ...
    //     await new Promise(r => setTimeout(r, 4000));
        var blob = new Blob([oEvent.target.response], {type: this.parent.srcType});
        if (this.parent.video) {
            this.parent.video.src = URL.createObjectURL(blob);
            this.parent.finishedLoading = true;
            this.parent.timeFinishedLoading = new Date();
            console.log("video finished downloading");
        } else {
            console.log("video finished downloading but video has been released");
        }
        if (this.parent.isSpinnerVisible) {
            this.parent.isSpinnerVisible = false;
            $.mobile.loading("hide");
        }
    };
    xhr.send();
...
VideoPlayer.prototype.canPlay = function () {
    return this.video && this.finishedLoading;
}

VideoPlayer.prototype.freeVideo = function () {
    if (this.xhr) {
